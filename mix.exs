defmodule ExUndercover.MixProject do
  ...
  defp deps do
    [
      ...
      # ecto_sql dev-only so `mix ecto.migrations` exists in dev (no Repo configured → no-op).
      # Not compiled into the library itself, so no conflict when stuffix uses ex_undercover as a path dep.
